        ...
        while (true) {
            try {
                ...
                BufferedReader in = new BufferedReader(
                        new InputStreamReader(playerController.getPlayer(playerId).getSocket().getInputStream()));
                String request = in.readLine();
                // Send the response to the oputput stream: server → client
                PrintWriter out = new PrintWriter(playerController.getPlayer(playerId).getSocket().getOutputStream());
                if (request.equals("Create game")) {
                    game = new Game();
                    game.setBoardArray(new int[15][15]);
                    game.setName("custom game");
                    game.setPlayer1(playerController.getPlayer(playerId));
                ...
                if (request.equals("Join game")) {
                    if (gameController.getGames().isEmpty()) {
                        ...
                        game.setPlayer2(playerController.getPlayer(playerId));
